...
#define cpu_monitor_LOG(fmt, ...)           do{ cpu_monitor_LOG_PRINTF_IMPL(cpu_monitor_LOG_COLOR_GREEN "[*]: " fmt cpu_monitor_LOG_END, ##__VA_ARGS__); } while(0)

#define cpu_monitor_LOGT(tag, fmt, ...)     do{ cpu_monitor_LOG_PRINTF_IMPL(cpu_monitor_LOG_COLOR_BLUE "[" tag "]: " fmt cpu_monitor_LOG_END, ##__VA_ARGS__); } while(0)
#define cpu_monitor_LOGI(fmt, ...)          do{ cpu_monitor_LOG_PRINTF_IMPL(cpu_monitor_LOG_COLOR_YELLOW "[I]: %s: " fmt cpu_monitor_LOG_END, cpu_monitor_LOG_BASE_FILENAME, ##__VA_ARGS__); } while(0)
#define cpu_monitor_LOGW(fmt, ...)          do{ cpu_monitor_LOG_PRINTF_IMPL(cpu_monitor_LOG_COLOR_CARMINE "[W]: %s: %s: %d: " fmt cpu_monitor_LOG_END, cpu_monitor_LOG_BASE_FILENAME, __func__, __LINE__, ##__VA_ARGS__); } while(0)
#define cpu_monitor_LOGE(fmt, ...)          do{ cpu_monitor_LOG_PRINTF_IMPL(cpu_monitor_LOG_COLOR_RED "[E]: %s: %s: %d: " fmt cpu_monitor_LOG_END, cpu_monitor_LOG_BASE_FILENAME, __func__, __LINE__, ##__VA_ARGS__); } while(0)
#define cpu_monitor_LOGF(fmt, ...)          do{ cpu_monitor_LOG_PRINTF_IMPL(cpu_monitor_LOG_COLOR_CYAN "[!]: %s: %s: %d: " fmt cpu_monitor_LOG_END, cpu_monitor_LOG_BASE_FILENAME, __func__, __LINE__, ##__VA_ARGS__); cpu_monitor_LOG_EXIT_PROGRAM(); } while(0)
...
#else
#define cpu_monitor_LOGD(fmt, ...)          do{ cpu_monitor_LOG_PRINTF_IMPL(cpu_monitor_LOG_COLOR_DEFAULT "[D]: %s: " fmt cpu_monitor_LOG_END, cpu_monitor_LOG_BASE_FILENAME, ##__VA_ARGS__); } while(0)
#endif
